...
test('build gera páginas HTML em dist/', () => {
  ...
  assert.ok(fs.existsSync(path.join(DIST, 'contratacao', 'index.html')));
});
...
test('build copia assets públicos', () => {
  ...
  assert.ok(fs.existsSync(path.join(DIST, 'js', 'contratacao', 'app.js')));
  assert.ok(fs.existsSync(path.join(DIST, 'css', 'pages', 'contratacao.css')));
